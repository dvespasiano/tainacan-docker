...
WEB_APP_PATH=/www
EOF
            ./conf/scripts/clonew.sh
            sudo docker-compose -f docker-compose-devw.yml up --build
            exit
        ;;
        --build-image)
            echo "[BUILD IMAGE]"
            sudo docker-compose -f docker-compose-devw.yml build
            exit
        ;;
        --build-image-nginx)
        ...
        --build)
            echo "[BUILD TAINACAN]"
            ./conf/scripts/build_tainacanw.sh --build
            exit
        ;;
        --watch-build)
        ...
        --stop)
            echo "[STOP TAINACAN]"
            sudo docker-compose -f docker-compose-devw.yml down
        ...
        --start)
            echo "[START TAINACAN]"
             docker-compose -f docker-compose-devw.yml up
            exit
        ;;
        --start-nginx)
